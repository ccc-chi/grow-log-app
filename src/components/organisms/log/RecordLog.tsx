import { Box, Text, Select, Button, Stack, Flex } from "@chakra-ui/react";
...
export const RecordLog: FC = memo(() => {
  ...
  const onClickRecordTimer = () => {
    if (!isRecording) {
      ...
      const newLog = [...recordLogs,{ start: currentStartTime, end : end, diffMs: diffMs }];
  ...
  useEffect(() => {
    ...
    const timerId = setInterval(() => {
      ...
      setStopwatchStr( `${hours.toString().padStart(2,"0")}:${minutes.toString().padStart(2, "0")}:${seconds.toString().padStart(2,"0")}`)
  ...
  return (
    <>
      <Box p={3} borderWidth={1} borderRadius="md" mb={10} backgroundColor={"gray.50"} maxWidth={"600px"}>
        <form>
          <Stack spacing={2}>
            <Text as="h2" fontWeight={"bold"} fontSize="l" my={2}>記録を開始する</Text>
            <Select placeholder='タスクを選択' backgroundColor={"white"}>
              <option value='option1'>Option 1</option>
              <option value='option2'>Option 2</option>
              <option value='option3'>Option 3</option>
            </Select>
            <Text fontSize={"4xl"} fontWeight={"bold"} textAlign={"center"}>{stopwatchStr}</Text>
            <Button colorScheme='teal' width={"100%"} onClick={()=> onClickRecordTimer()}>
              {isRecording ? "記録を停止" : "記録を開始"}
            </Button>
            <Box mt={5}>
              <Stack spacing={2} backgroundColor={"White"} borderWidth={1} borderRadius="md" p={4}>
                <Text fontWeight={"bold"}>タスクに記録する</Text>
                <Text fontSize={"xs"} mb={2}>登録したいタスクが表示されていることを確認し、進捗を入力してください。</Text>
                <Text>Option 1</Text>
                <Text>記録時間：{totalTimeStr}</Text>
                <Box gap={0} mb={2}>
                  <Text>進捗</Text>
                  <Flex alignItems={"flex-end"}>
                    {/* <Input value={progress} w={"50px"} /> */}
                    <Text>%</Text>
                  </Flex>
                </Box>
                <Button colorScheme='teal' width={"100%"} onClick={()=> setIsRecording(!isRecording)}>
                  登録
                </Button>
              </Stack>
            </Box>
